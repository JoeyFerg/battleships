    ...
    void positionShip(ship* type){
        while(1){
            ...
            if (thisBoardOP->getSpace(X,Y)=='~'){                                       //Ship can only be placed in the sea ('~')
                if (!type->buildShip(&X,&Y,dX, dY,thisBoardOP)){
                    dX*=-1;
                    dY*=-1;
                    if (type->buildShip(&X,&Y,dX,dY,thisBoardOP)){return;}        //try again in the other direction
                }else{return;}
            }
        }
    }
...
public:
    ...
    void go(){
        ...
        if(hit()){
            if(originX==0){     					//first hit on a ship
                originX=X;
                originY=Y;
            }else if(!foundDirection){    	//found direction of ship (second hit)
                foundDirection=true;
                if(oppositeCheck){
        ...
        }else if(foundDirection){                         //missed a particular ship
            if (X-originX>0||Y-originY>0){foundEnd1=true;		//upper or right end found (+)
    ...
    void setXY(){
        if(originX!=0){                         //ship has been hit
            if(!foundEnd1 || !foundEnd2){
                if(!foundDirection){		//direction of ship has not been found yet
                    findDirection();
                }else{                          //direction of ship has been found
                    findRest();
                }
            }
            if(foundEnd1 && foundEnd2){		//entire ship has been destroyed (at least that's what it looks like)
    ...
    void findRest(){                //sets X & Y when the direction of the ship is known
        int adjustment=0;
        if(foundEnd1){                                //upper or right direction blocked
            if(vX<0||vY<0){adjustment=1;
            }else{adjustment=-distance;}
        }else if(foundEnd2){                          //lower or left direction blocked
        ...
        int i=0;
        while(i<2&&(!foundEnd1 || !foundEnd2)){                               //loop if its blocked on one side
